...
> **⚠️ 慎用提示**：数模竞赛中的"预测问题"数据量通常 < 500 条，**严重不适合神经网络 / LSTM**。使用时几乎必定过拟合，反而扣分。
>
> - 数据量 < 500：优先用**灰色预测 / ARIMA / 回归 / 插值拟合**
> - 数据量 500-5000：可考虑 XGBoost / LightGBM（加正则 + 交叉验证）
> - 数据量 > 5000：才值得上 LSTM / 神经网络
>
> 详见 `算法选择决策指南.md` §2 和 §4 反模式清单。**不要为了"显得高级"硬上神经网络**，评委会判为"方法与问题不匹配"。
