...
- 图表类型应多样化，包括但不限于：
  ...
  - 网络图、树状图（根据问题类型）
- 图表应清晰展示关键结果和发现
- 所有图表保存到 `figures/` 目录，优先 PDF 格式（矢量），其次 dpi=300 PNG
- 图片命名规范：`q{问题号}_fig{序号}.pdf`
...
19. **图片数量**: 每个问题至少生成 4 张图表，越多越好，确保正文 <= 25 页的前提下尽量丰富
20. **图片格式**: 优先保存为 PDF（矢量图），其次 300dpi PNG。命名：`q{问题号}_fig{序号}.pdf`
